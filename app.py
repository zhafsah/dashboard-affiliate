...
if tombol_proses:
    if not uploaded_files:
        ...
        if len(list_df_sales) > 0:
            ...
            df_meta = pd.concat(list_df_meta, ignore_index=True) if len(list_df_meta) > 0 else pd.DataFrame()
            ...
            if not df_meta.empty:
                ...
                ad_tags = set()
                meta_sum = pd.DataFrame(columns=['Clean_Tag', 'Spend', 'Klik_Meta'])
            ...
            merged = pd.merge(meta_sum, click_sum, on='Clean_Tag', how='outer') if not meta_sum.empty else click_sum
            merged = pd.merge(merged, sales_sum, on='Clean_Tag', how='outer').fillna(0)

            if 'Spend' not in merged.columns: merged['Spend'] = 0.0
            if 'Klik_Meta' not in merged.columns: merged['Klik_Meta'] = 0
